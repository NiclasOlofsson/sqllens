...
const AGGREGATES = new Set([
  "sum", "count", "count_big", "avg", "min", "max", "stdev", "stdevp", "var", "varp", "grouping",
  "grouping_id", "string_agg", "checksum_agg", "approx_count_distinct",
]);
...
export function lower(tree: ParserRuleContext): QueryExpr {
  ...
  const ctes = ctesNode
    ? directChildrenOfRule(ctesNode, P.RULE_common_table_expression).map(lowerCte)
    : [];
...
function lowerQueryExpression(query: ParserRuleContext): QueryBody {
  ...
  if (!body) {
    ...
    for (let i = 1; i < inners.length; i++) {
      body = {
        kind: "setop", op: "union", all: hasToken(query, P.ALL),
        left: body, right: lowerQueryExpression(inners[i]), columns: [], cst: query,
      };
    }
  }
  ...
  for (const u of directChildrenOfRule(query, P.RULE_sql_union)) {
    body = {
      kind: "setop", op: setOpKind(u), all: directTokenType(u, [P.ALL]) !== undefined,
      left: body, right: lowerUnionBranch(u), columns: [], cst: query,
    };
  }
  return body;
}
...
function buildSelect(spec: ParserRuleContext): SelectExpr {
  ...
  return {
    kind: "select", projections, from, columns, where: whereExpr,
    joinConditions: joinConditions.length ? joinConditions : undefined,
    groupBy, having, aggregated,
...
function buildProjection(elem: ParserRuleContext): Projection {
  ...
  if (asterisk) {
    ...
    return {
      name: undefined, isStar: true,
      expr: { kind: "star", qualifier: qn ? nameParts(qn) : undefined, cst: asterisk }, cst: elem,
    };
  }
...
function buildSource(item: ParserRuleContext): Source {
  ...
  if (derived) {
    ...
    return {
      ...
      alias: alias?.text, aliasCst: alias?.cst,
...
function lowerPredicate(pred: ParserRuleContext): Expr {
  ...
  if (hasDirectToken(pred, P.BETWEEN)) {
    return { kind: "predicate", op: "between", negated, operand, args: exprs.slice(1).map(lowerExpression), cst: pred };
  }
  ...
  if (hasDirectToken(pred, P.LIKE)) {
    return { kind: "predicate", op: "like", negated, operand, args: exprs.slice(1, 2).map(lowerExpression), cst: pred };
  }
...
function lowerExpression(node: ParserRuleContext): Expr {
  switch (node.ruleIndex) {
    ...
    case P.RULE_expression: {
      ...
      if (exprs.length === 2) {
        return { kind: "binary", op: binaryOp(node), left: lowerExpression(exprs[0]), right: lowerExpression(exprs[1]), cst: node };
      }
...
function firstExprChild(node: ParserRuleContext): ParserRuleContext | undefined {
  const rules = [
    P.RULE_primitive_expression, P.RULE_function_call, P.RULE_case_expression, P.RULE_full_column_name,
    P.RULE_bracket_expression, P.RULE_unary_operator_expression, P.RULE_expression,
  ];
...
function lowerCase(node: ParserRuleContext): Expr {
  ...
  if (searchedSecs.length > 0) {
    ...
    return { kind: "case", whens, elseExpr: directExprs[0] ? lowerExpression(directExprs[0]) : undefined, cst: node };
  }
...
function columnsOf(expr: Expr, acc: ColumnRef[], clause: Clause): void {
  switch (expr.kind) {
    case "column": acc.push({ parts: expr.parts, clause, cst: expr.cst }); break;
    case "binary": columnsOf(expr.left, acc, clause); columnsOf(expr.right, acc, clause); break;
    case "unary": columnsOf(expr.operand, acc, clause); break;
    case "cast": columnsOf(expr.expr, acc, clause); break;
    case "function":
      ...
      break;
    case "case":
      expr.whens.forEach((w) => { columnsOf(w.when, acc, clause); columnsOf(w.then, acc, clause); });
    ...
    case "predicate":
      ...
      expr.args.forEach((a) => columnsOf(a, acc, clause));
      break;
    case "subscript": columnsOf(expr.base, acc, clause); columnsOf(expr.index, acc, clause); break;
    case "other": cstColumnRefs(expr.cst, acc, clause); break;
...
function hasAggregate(expr: Expr): boolean {
  switch (expr.kind) {
    case "function": return (expr.aggregate && !expr.window) || expr.args.some(hasAggregate);
    case "binary": return hasAggregate(expr.left) || hasAggregate(expr.right);
    case "unary": return hasAggregate(expr.operand);
    case "cast": return hasAggregate(expr.expr);
    case "case":
      return expr.whens.some((w) => hasAggregate(w.when) || hasAggregate(w.then)) ||
        (expr.elseExpr !== undefined && hasAggregate(expr.elseExpr));
    case "predicate": return hasAggregate(expr.operand) || expr.args.some(hasAggregate);
...
function stripQuotes(text: string): string {
  if (text.length >= 2) {
    const a = text[0], z = text[text.length - 1];
    if ((a === "[" && z === "]") || (a === '"' && z === '"') || (a === "`" && z === "`")) {
