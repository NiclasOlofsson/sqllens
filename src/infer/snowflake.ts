...
export const SNOWFLAKE_FUNCTION_RETURNS: Record<string, FnRule> = {
	...
	...group(fixed(BIG), ["hash", "md5_number_lower64", "md5_number_upper64", "seq1", "seq2", "seq4", "seq8", "random"]),

	// --- numeric ----------------------------------------------------------------
	...group(firstArg, ["abs", "ceil", "floor", "round", "trunc", "truncate", "sign", "bitnot", "bitand", "bitor", "bitxor"]),
	mod: common,
	...group(fixed(D), [
		"acos",
		"acosh",
	...
	// --- conversion ----------------------------------------------------------------------
	...group(fixed(DEC), ["to_number", "to_numeric", "to_decimal", "try_to_number", "try_to_numeric", "try_to_decimal"]),
	to_double: fixed(D),
	...
	...group(fixed(OBJECT), ["object_construct", "object_construct_keep_null", "object_insert", "object_delete", "object_agg"]),
	object_keys: fixed({ kind: "array", element: S }),
	strtok_to_array: fixed({ kind: "array", element: S }),
	split: fixed({ kind: "array", element: S }),
	...group((args) => (args[0]?.kind === "array" ? args[0] : { kind: "array", element: UNKNOWN }), [
		"array_append",
		"array_cat",
		"array_compact",
		"array_distinct",
