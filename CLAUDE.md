...
Current state only, verified 2026-06-10 (all suites green, 290 tests at last full run — counts move while the Snowflake dialect is built in a parallel session). History lives in `git log` and the PLAN.md Open Gaps entries — don't append changelog narrative here; keep this section true to *now*.

- **Phase 0 done; Databricks and T-SQL are at their gates; the Phase 1.5 semantic layer is built and wired.** Next: the warehouse dialects — Redshift → Snowflake → BigQuery (hand-authored; Snowflake is being built in a parallel session; PLAN.md Phases 3–5 still hold stale Redshift-first detail, pending a re-sequence).
...
- **Type inference is per-dialect** — the engine (`src/infer/infer.ts`) is dialect-agnostic; per-dialect knowledge lives in `src/infer/dialect.ts`: function registries, literal rules, scalar-type aliases (bit→boolean, datetime→timestamp, nvarchar→string, float→double), and per-dialect division (Spark `int/int`→double, T-SQL `int/int`→int). The dialect rides as a string tag on `Scope` (`resolveScopes(query, "tsql")`), so qualify/symbols needed no change. The registries are built from the vendor references with doc-fetched return types (Spark ~520 entries incl. the H3/ST-geospatial/AI/IP/VARIANT/TIME families; T-SQL ~210 incl. the 2022/2025 additions); a rule is absent only where the documented return type is argument-value-dependent or unstated. Gates: `tsql.infer.test.ts`, `infer.registry.test.ts`.
