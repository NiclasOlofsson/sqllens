...
src/fragment.ts            openFragments(text, dialect) — parse ranges of a text as an SQL fragment (statement | expression | tableSource | cteList | selectList) over one lex, driving the grammar's own rules (no wrapper rules in the .g4: an extra EOF caller widens SLL follow sets), bound per dialect in src/<dialect>/parse.ts (defineFragmentGrammar, src/fragment-grammar.ts). Consumer: minijinja macro bodies
src/minijinja/             the additive jinja-SQL front end — segment.ts (outer-jinja segmenter + length/newline-preserving placeholders), parse.ts (parseTemplated/tokenizeTemplated), tag-ast.ts (ref/source/macro TagNodes), apply-tags.ts ({{ ref }}/{{ source }} in a FROM slot → a real TableSource.template), regions.ts (control-flow region tree + go-to-def symbols; a macro region carries its body's fragment verdict), variants.ts (arm-coverage branch enumeration); parse.ts also derives `macros: MacroShape[]` (what each `{% macro %}` body can stand in for, + `shapesForCall`) so a host's `shapeOf` is one index lookup. Total; reachable only through the barrel; the SQL grammars are untouched
...
src/document/              the living-document model — document.ts (SqlDocument), split.ts (splitStatements: the token-level statement-cell splitter, depth-aware, each cell carrying its separator), spans.ts (statementSpans: the cell spans without the parses), line-index.ts (LineIndex: O(log n) position↔offset), node-at.ts (CST node at an offset)
