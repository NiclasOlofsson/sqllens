...
- **MySQL `quantifiedSubqueryAtom` residual quirk.** ~~In the semicolon-less statement-FINAL position, `... WHERE b > ANY (SELECT 1)<EOF>` mis-splits into two statements; root fix needs the WITH/SELECT adjacency restructure first...~~ RESOLVED (2026-07-11, `fix(mysql): require SEMI between batch statements`). `sqlStatements` now requires a `SEMI` between statements, with a new `statementItem : withStatement sqlStatement | sqlStatement | emptyStatement_` admitting the one no-semicolon adjacency real MySQL has (a `WITH` clause bound to its query — the shape upstream models as two adjacent statements); `lower()` flattens `statementItem` and rejoins the WITH/SELECT pair, now unambiguously (`WITH ...; SELECT ...` with a semicolon between is no longer merged). A language contraction to match real MySQL (the client splits batch input on the `;` delimiter — dev.mysql.com/doc/refman/8.4/en/mysql-batch-commands.html): the `ANY`/`SOME` quantified comparison parses as one query with no trailing `;`, and `SELECT 1 SELECT 2` now correctly errors. Historical note: the quirk was inherited from upstream's optional `SEMI?` separator, and 2 grammars-v4 fixtures leaned on that over-acceptance — an escalation round first proposed KNOWN_BAD-quarantining them, overruled in favor of fixing the fixtures: `grant.sql` (140 delimiter-less GRANT/REVOKE statements) and `ddl_create.sql` (18 unterminated trigger/procedure/function bodies) were corrected to valid MySQL in the corpus repo (`c2623b89`, every original statement retained), keeping their full coverage in the gate.
- **MySQL docs-tier SLL floor (~~612~~ → 33/1252 files fall back).** Re-measured 2026-07-11 after the SEMI-required batch restructure: 33, down from 612 — the SEMI requirement was the only change between the measurements, so the optional statement separator (an SLL conflict at nearly every statement boundary) was the real driver, not the function-call/UNION-chain shapes previously blamed. The residual 33 concentrate in WITH/CTE-shaped files (14 under `query/with` — the `withStatement`→query adjacency is the one no-semicolon statement pair the batch rule still admits) and set-operation chains (5 under `set-operations`/`union`), plus a 14-file singleton tail. The grammars-v4 floor moved 11→6 the same way. SLL surgery on the WITH-adjacency prediction is the remaining follow-up (`tests/corpus/mysql.test.ts`'s floor comments).
