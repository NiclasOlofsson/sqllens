...
- **Symbol model — STARTED 2026-06-06 (`src/symbols/`).** A SQL-native symbol model derived from the scope tree: `Sym { kind, modifiers, name, span, frame }`, a **kind × modifier** taxonomy (kinds: table/view/cte/subquery/lateral/column/alias/function/parameter/literal/keyword; modifiers: declaration/reference/output/aggregate/window/correlated/star). First cut covers **relation** symbols (table/CTE/subquery references + CTE declarations) and **column** symbols (references, alias/computed output declarations, `*`, and `correlated` via `resolveColumn`). `deriveSymbols` runs over all 1558 models with 0 throws (92266 symbols). **Next slices:** function/aggregate/window symbols + parameter/keyword; column **provenance** (which source/expression a column derives from) once it's worth carrying; **types on column symbols** (needs type inference); projections to LSP `DocumentSymbol`/`SemanticTokens` and the debugger's `@dbg` frames.
