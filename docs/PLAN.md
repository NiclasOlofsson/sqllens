...
- **Expression-completeness (`other`-node) ratchet — measured for all five dialects, tracked falling-only.** Each dialect's `tests/corpus/*.test.ts` pins the current `other`-expression count over its docs/corpus and fails if it rises (riding the same single parse the parse-gate makes); Databricks stays gated at 0/1558 in `tests/corpus/databricks.oatly.test.ts`. (The former standalone `tests/ir-completeness.dialects.test.ts` was folded into the per-dialect corpus files and deleted.) 2026-07-01 baselines and their top leak classes: **BigQuery 234/14,695** — mostly braced/proto/struct constructors (`{f:v}`, `STRUCT{}`/`NEW T{}`, `replace_fields(...)`) and `WITH(x AS ..., x)` expression forms not yet lowered; **T-SQL 26/1,555** — XML `.value('(@name)[1]', 'type')` calls and `REGEXP_LIKE(...)` predicate forms; **Snowflake 10/2,976** — sequence object references (`seq.NEXTVAL`-style `Object_name`s); **Redshift 0/1,808** — already expression-corpus-complete. (The T-SQL/Snowflake/Redshift denominators grew from the corpus path-bucketing reorg below — the same `other` count over a larger, correctly-classified query population.) Each is a real gap to close, not a floor to relitigate downward.
- **Statement-kind parity + corpus path-bucketing — DONE.** All five dialects export `statementCategories` (per-statement kind detection over the parse tree); Redshift's now classifies structurally by `stmt` rule name instead of a leading-keyword guess, and walks only the top-level `stmtmulti` children so a `CREATE FUNCTION … BEGIN ATOMIC …` body doesn't double-count. The Databricks/T-SQL/Snowflake/Redshift docs corpora were reorganized into one legible `<dialect>/<source>/<stage>/<validity>/<category>/…` tree (`tools/organize-corpus.test.ts`, `ORGANIZE=1`), pre-sorted by `statementCategories` under the current parser; the gates trust the path instead of reclassifying at test time, so `tests/helpers/sql-kind.ts` (the old leading-keyword regex) and T-SQL's parse-everything classify mode are both deleted. Query-bucket populations, all 100% parse: Databricks 3,088, T-SQL 1,555, Snowflake 2,976, Redshift 1,808 — the growth over the pre-reorg counts is files the regex had misbucketed into dml/ddl and that were never gated.
- **BigQuery inference registry — 351 doc-cited entries.** Built out from a 184-entry starter to cover all 30 documented GoogleSQL function families; 351 is the adjudicated determinable ceiling under the project's never-wrong contract (value-dependent/TVF/AI-function returns stay `unknown` by design, not a gap to close). Breadth floor pinned in `tests/infer.registry.test.ts`.
...
- **Inference** — `src/infer/bigquery.ts` (INT64/FLOAT64/NUMERIC/BOOL/BYTES/JSON aliases, typed-literal rules, INT64/INT64→FLOAT64 division, a 352-function GoogleSQL return registry — see Open Gaps).
