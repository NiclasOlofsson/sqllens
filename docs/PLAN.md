...
- **Symbol model — STARTED 2026-06-06 (`src/symbols/`).** A SQL-native symbol model derived from the scope tree: `Sym { kind, modifiers, name, span, frame }`, a **kind × modifier** taxonomy (kinds: table/view/cte/subquery/lateral/column/alias/function/parameter/literal/keyword; modifiers: declaration/reference/output/aggregate/window/correlated/star). Built so far: **relation** symbols (table/CTE/subquery references + CTE declarations), **column** symbols (references, alias/computed output declarations, `*`, `correlated` via `resolveColumn`), and **function** symbols (with `aggregate`/`window` modifiers, from the expression trees). `deriveSymbols` runs over all 1558 models with 0 throws.
  **NOT done (open):**
  - `alias`, `parameter`, `keyword`, `literal` kinds are declared in the taxonomy but **never emitted** — e.g. `t AS x` produces no `alias` symbol (the alias is folded into the column).
  - **No definition→reference link** — a column reference's binding is computed by `resolveColumn` but not carried on the symbol, so there is no go-to-definition edge yet.
  - **No provenance** (which source/expression a column derives from) and **no types** on column symbols (types need type inference, itself open).
  - **No projections** — the model is internal-only; nothing maps it to LSP `DocumentSymbol`/`SemanticTokens` or the debugger's `@dbg` format, so no consumer uses it yet. (Highest-value gap.)
  - Minor: scalar/IN/EXISTS subqueries get a generic `_sub_` frame label; ORDER BY expressions aren't walked for function symbols.
