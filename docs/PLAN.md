...
- **DuckDB `UNION BY NAME` drops the `byName` IR flag.** The grammar parses `UNION [ALL] BY NAME`, but `src/duckdb/lower.ts` builds the `setop` node without setting `byName` (the scope layer approximates name-matching positionally). Snowflake and Trino model the flag (`byName: true` on the set-op IR), so a genuinely by-name-matching consumer — e.g. per-hop lineage across legs with different column orders — works on those two and not DuckDB. Additive teach-in: set `byName` on the DuckDB `setop` when the `by_name` child is present, mirroring `src/snowflake/lower.ts`. Surfaced by the per-hop lineage work (consolidation wave), whose BY-NAME acceptance case routes through Snowflake for exactly this reason.
