...
**Out — actually cleared by Nicke:** SQL transpilation ("i dont care at all about the transpile"); and, **cleared 2026-06-10** ("you are correct on the usecase for the parser"), the **Databricks platform/admin statement grammar** — COPY INTO, the Delta utilities (OPTIMIZE/VACUUM/RESTORE/CLONE/REORG/FSCK), Unity Catalog DDL (CREATE CATALOG/VOLUME/EXTERNAL LOCATION/SHARE/CONNECTION, column MASK/ROW FILTER, CLUSTER BY AUTO), GRANT/security, and `LANGUAGE PYTHON $$…$$` bodies. The parser's use case is queries + their semantic layer; these statements stay rejected and are pinned as `noparse` entries in `tests/databricks.doc-coverage.test.ts` (flip an entry's flag in the change that adds one). A query engine is out by definition (this is a parser, not an execution engine). **NOT cleared — open, do NOT treat as Out** (a prior edit wrongly stamped these "Nicke-cleared"; corrected 2026-06-06): **type inference** and **column lineage** (lineage was only noted "revisit later", rides on qualify). **Amended 2026-06-06:** name resolution (**scope**) and column/`*` resolution against a supplied schema (**qualify**) are **in scope for Databricks** as a semantic layer on the parse tree (Phase 1.5) — the consumers (editor support, the SQL debugger) need them. The warehouse dialects get the grammar only until a second consumer forces the abstraction.
