...
- **Struct/field dot-access — FIXED 2026-06-06.** `resolveColumn`/`qualify` no longer assume `parts[-2]` is the qualifier. `splitColumnRef` splits a dotted ref into qualifier / column / field-path against the visible sources (a leading part is a qualifier only if it names a source, else it's the column and the rest is field navigation — Spark's resolution order). `t.addr.city` binds to `t` with column `addr`, fields `[city]`; unqualified `addr.city` binds to the column `addr`. Corpus schema-free `unresolved` dropped 44→33. **Residual (OPEN GAP — NOT cleared):** verifying a struct *field* exists (`city` in `addr`) is unbuilt — qualify's `Schema` models columns as flat `name → type` and doesn't decompose struct types into their fields, so field-level access isn't validated. This is unfinished qualify/Schema work that needs struct field-type info — related to **type inference**, which is itself open, NOT cleared (see Scope). Subscript/colon forms (`col['k']`, `arr[0]`, variant `v:a.b`) lower to `other` and recover only the base column (the key/index isn't a column) — acceptable, no mis-binding.
