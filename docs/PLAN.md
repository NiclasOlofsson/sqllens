...
- **Per-hop lineage — SPEC (Anvil ITEM 4, decided with Niclas 2026-07-03; NOT yet built).** The hop-chain API the extension's Lineage & Impact panel and "go to origin" navigation need: not a new node datatype mimicking sqlglot's `Node` graph, but a **traversal-order spine of references into the structures we already have** — every hop points at the frozen, span-carrying nodes the walk visits, so navigation is span-jumping and there is no parallel model to keep in sync ("a filtered view on the already parsed tree, one per qualifiable column" — Niclas). Shape:
  ```ts
