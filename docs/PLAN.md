...
- **BigQuery inference registry — 352 doc-cited entries.** Built out from a 184-entry starter to cover all 30 documented GoogleSQL function families; 352 is the adjudicated determinable ceiling under the project's never-wrong contract (value-dependent/TVF/AI-function returns stay `unknown` by design, not a gap to close). Breadth floor pinned in `tests/infer.registry.test.ts`.
