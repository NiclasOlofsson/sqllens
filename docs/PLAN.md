...
- **Expression modelling — BUILT 2026-06-06 (residual below).** `lowerExpression` produces a typed `Expr` tree for every select / `WHERE` / `HAVING` / `GROUP BY` expression: column, literal, star, binary, unary, function (with aggregate + window/`OVER` detection), `CASE`, cast, scalar subquery, `EXISTS`; plus `SelectExpr.where`/`groupBy`/`having` and an `aggregated` flag. Anything unmodelled is an explicit `other` node, never dropped. Corpus: **98.9%** of 94k expr nodes modelled. **Residual gaps (still `other` / partial, tracked):** predicates (`IS NULL`, `IN`, `BETWEEN`, `LIKE`) lower to `other`; grouping analytics (ROLLUP/CUBE/GROUPING SETS) keep only their first expression; join `ON` conditions are still flat `ColumnRef`s, not `Expr`; the `aggregate` flag is a function-name heuristic, not a catalog.
