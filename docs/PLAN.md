...
- **Expression modelling — BUILT 2026-06-06 (residual below).** `lowerExpression` produces a typed `Expr` tree for every select / `WHERE` / `HAVING` / `GROUP BY` expression: column, literal, star, binary, unary, function (with aggregate + window/`OVER` detection), `CASE`, cast, scalar subquery, `EXISTS`; plus `SelectExpr.where`/`groupBy`/`having` and an `aggregated` flag. Anything unmodelled is an explicit `other` node, never dropped. Corpus: **98.9%** of 94k expr nodes modelled. `SelectExpr.columns` is derived from the `Expr` trees (projections, WHERE, JOIN `ON`, GROUP BY, HAVING, ORDER BY) — single source of truth, with a CST fallback for `other` nodes so nothing is lost. A **CST↔IR conservation gate** (`tests/conservation.test.ts`) runs over all 1558 models and fails if the IR drops any clause the parse tree contains. **Residual gaps (still `other` / partial, tracked):** predicates (`IS NULL`, `IN`, `BETWEEN`, `LIKE`) lower to `other`; grouping analytics (ROLLUP/CUBE/GROUPING SETS) keep only their first expression; the `aggregate` flag is a function-name heuristic, not a catalog.
...
- **Struct/field dot-access — FIXED 2026-06-06.** `resolveColumn`/`qualify` no longer assume `parts[-2]` is the qualifier. `splitColumnRef` splits a dotted ref into qualifier / column / field-path against the visible sources (a leading part is a qualifier only if it names a source, else it's the column and the rest is field navigation — Spark's resolution order). `t.addr.city` binds to `t` with column `addr`, fields `[city]`; unqualified `addr.city` binds to the column `addr`. Corpus schema-free `unresolved` dropped 44→33. **Struct field-existence validation — BUILT 2026-06-06.** `parseStructFields` (schema.ts) parses `struct<…>` type strings, nesting-aware; qualify's `checkFieldPath` walks the field path against the base column's struct type and emits an `unknown-field` diagnostic when a known struct lacks the field (`t.addr.city` → checks `city` in `addr`'s struct; nested `a.b.c` walks down). Conservative: only fires when the base column's struct type is known from a **table schema**; a non-struct, array/map, or unknown type stops the walk without flagging. **Residual (OPEN GAP):** types aren't propagated through CTE/subquery outputs (those carry names, not types), so field access on a *derived* column isn't validated; and array/map element access (`m['k'].f`, `arr[0].f`) isn't modelled — the subscript lowers to `other`, dropping the field path. Subscript/colon forms (`col['k']`, `arr[0]`, variant `v:a.b`) recover only the base column (the key/index isn't a column) — no mis-binding.
