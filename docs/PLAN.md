...
- **Jinja — one TagNode per tag (inc1 boundary).** `tagNodesOf` returns exactly one `TagNode` per jinja tag: the leftmost-topmost call in the tag's parse tree. A tag holding two sibling calls (`{{ [ref('a'), ref('b')] }}`) yields only the first; an arithmetic tag with an embedded call (`{{ x + ref('y') }}`) classifies off that call. Rare in real dbt (a FROM is a lone `{{ ref() }}`) and the emitted spans stay accurate for the returned node — but a multi-call tag under-reports. Retired when inc2 needs multi-node tags. Also deferred from the spec: the syntactic-slot context field on the tag node (the inc1 `TagNode` is the R2 span contract only). Full jinja design + increment plan: [jinja-front-end.md](jinja-front-end.md).

## Jinja-SQL front end (ITEM 10/11/14) — inc1 built, inc2/inc3 spec
...
`JINJA-CONSUMPTION-PLAN.md`). minijinja is the grammar oracle. **inc1 built:**
`parseTemplated`/`tokenizeTemplated` (R1 unified stream — SQL ch 0/1 + jinja ch 2, role `"jinja"`) + the
R2 ref/source/macro tag-AST + the `tests/corpus/jinja.test.ts` gate, additive over the untouched grammars
(barrel-only reach). inc2/inc3 spec'd — see docs/jinja-front-end.md.
