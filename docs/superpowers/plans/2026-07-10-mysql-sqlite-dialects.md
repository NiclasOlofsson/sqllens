...
- **MySQL `scalarFunctionName` reserved-word audit follow-up.** `IF`/`REPLACE`/`INSERT` sit in `scalarFunctionName` as reserved keywords that also double as function-call names; `CONVERT` gets its own dedicated `dataTypeFunctionCall` alternative for the same reason (CAST-like syntax, not an ordinary call). Worth a systematic audit for other MySQL-reserved words that are valid function names but missing from (or mishandled around) this class — not done here.
