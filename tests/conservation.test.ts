...
function countOrderBy(tree: ParseTree): number {
  ...
  for (const d of descendants(tree)) {
    ...
    for (let i = 0; i < d.getChildCount(); i++) {
      ...
      if (c && !(c instanceof ParserRuleContext) && (c as { symbol?: { type: number } }).symbol?.type === P.ORDER) n++;
    }
  }
  return n;
}
  ...
  it("the IR drops no clause the CST contains, across all 1558 models", () => {
    ...
    console.log(
      [
        ...
        ...(Object.keys(offenders).length ? ["dropped (first offender):", ...Object.entries(offenders).map(([k, v]) => `  ${k}: ${v}`)] : []),
      ].join("\n"),
    );
    ...
    for (const k of Object.keys(cstTotal)) {
      expect(irTotal[k], `IR dropped some ${k} (CST ${cstTotal[k]} > IR ${irTotal[k]}) e.g. ${offenders[k]}`).toBeGreaterThanOrEqual(cstTotal[k]);
    }
  }, 180000);
});
